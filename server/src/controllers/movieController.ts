...
import { movies, nextId } from '../models/movie';
...
export const scanDirectory = async (req: Request, res: Response): Promise<void> => {
  try {
    const scannedMovies = await fileScanner.scanDirectoryForMovies(config.MOVIES_DIR);
